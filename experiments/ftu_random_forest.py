...
ethnicity_cols = [col for col in X.columns if 'ethnicity' in col]
X = X.drop(ethnicity_cols, axis=1)
...
for experiment_seed in tqdm(range(10)):
    ...
    for train_idx, test_idx in tqdm(cv_outer.split(X, y), leave=False):
        ...
        model = RandomForestClassifier(n_jobs=n_cpu-1, class_weight="balanced")
        search = RandomizedSearchCV(model, rf_space, scoring='f1', cv=cv_inner, refit=True, n_iter=10)
...
pd.DataFrame(metrics).to_csv("../experiments_metrics/ftu_random_forest_cleanv3.csv")